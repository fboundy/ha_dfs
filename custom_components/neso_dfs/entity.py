...
class DfsEntity(CoordinatorEntity[DfsCoordinator]):
    ...
    def __init__(self, coordinator: DfsCoordinator, key: str, participant: str | None = None) -> None:
        super().__init__(coordinator)
        self.tracked_participant = participant
        scope = f"_{slugify(participant)}" if participant else ""
        self._attr_unique_id = f"{DOMAIN}_zone{coordinator.zone}{scope}_{key}"
        if participant:
            self._attr_translation_placeholders = {"participant": participant}
